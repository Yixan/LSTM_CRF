...
# 获得一个句子中每个字对应的索引
def sentence2id_pos(sentences, word2id_dict):
    """
    :param sentences: 所有句子
    :param word2id_dict: 记录字和字对应索引的字典
    :return: 包含所有句子中每个字索引的List
...
def get_tag2label(file_location):
    with open(file_location, 'rb') as fr:
        tag2label_dict = pickle.load(fr)
    print('get_tag2label')
    return tag2label_dict

def get_label2tagcws(file_location):
    with open(file_location, 'rb') as fr:
        label2tag_dict = pickle.load(fr)
    print('get_label2tag')
    return label2tag_dict

# 获得一个句子中每个字对应的索引
def sentence2idcws(sentences, word2id_dict):
    """
    :param sentences: 所有句子
    :param word2id_dict: 记录字和字对应索引的字典
    :return: 包含所有句子中每个字索引的List
...
if __name__ == '__main__':
    # word2id = get_word_id('D:\\nlp\\LSTM_CRF\\data\\pos.pkl')
    # for key, id in word2id.items():
    #     if id == 0:
    #         print(key)
    # with open('D:\\nlp\\LSTM_CRF\\data\\trainset\\train_pos1.txt', 'r', encoding='utf-8') as fr:
    #     sentences=fr.readlines()
    #
    # sentence2id(sentences,word2id)
    tag2label = get_tag2label('D:\\nlp\\LSTM_CRF\\data\\val_pos_tag2num_dict.pkl')
    print(tag2label)